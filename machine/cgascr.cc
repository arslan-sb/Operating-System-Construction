...
{
	// There are no runtime bounds checks in C++, so we have to do them
	// ourselves -- writing past the video memory would silently corrupt
	// whatever happens to live behind it.
...
{
	// Deliberately no shadow copy of the position in the object: the
	// hardware cursor is the single source of truth, so the two can never
	// drift apart.  The price is two I/O accesses per query, which print()
	// pays exactly once per call.
	index_port.outb(CURSOR_HIGH);
...
void CGA_Screen::print(char *text, int length, unsigned char attrib)
{
	...
	for (int i = 0; i < length; i++) {
		...
		if (x >= COLUMNS) { // end of line reached: wrap
			x = 0;
			y++;
		}
...
void CGA_Screen::scrollup(unsigned char attrib)
{
	...
	// copy rows 1..ROWS-1 one row up (no memmove available -- freestanding)
	while (src < end)
		*dst++ = *src++;

	// blank the last row, which is still a duplicate of the one above it
	while (dst < end) {
		*dst++ = ' ';
		*dst++ = attrib;
	}
